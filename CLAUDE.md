...
An LLM-generated summary of why the user's portfolio moved, generated entirely on demand — there is no scheduled/background generation at all, so (unlike the other AI features) there's no settings toggle for it; there's nothing to opt out of costing anything until it's actually clicked. Home shows a one-line, zero-cost teaser (`PortfolioExplanationCard.tsx`, arrow icon hinting it's clickable); clicking it opens a command bar session that fetches or generates the actual explanation. `portfolio_explanations` is keyed by `(user_id, scope, scope_key, timeframe)` — one row per independent insight (`scope`: `'stock' | 'sector' | 'portfolio'`; `scope_key`: the symbol/theme name, or `''` for portfolio scope; `timeframe`: `'daily' | 'weekly' | 'monthly' | 'yearly' | 'custom'`, with `window_days` as the actual day-count behind it) — upserted in place per slot, no history table; `llm_usage_log` is where per-generation trends live. Today the UI only ever generates the single `('portfolio', '', 'daily')` slot (`DAILY_PORTFOLIO_SLOT` in `portfolioExplanation.ts`) via the teaser described below; the schema and generation logic already support the other slot types a Portfolio Pulse carousel will surface.

- **`PortfolioInsightSlot`** (`{ scope, scopeKey, timeframe, windowDays }`) identifies one insight. `computeMovers`/`buildTeaser`/the daily hysteresis logic are untouched and remain the exact basis for the shipped `('portfolio', '', 'daily')` card. `computeMoversForWindow(assets, netWorth, priceHistory, windowDays, timeframe)` is the weekly/monthly/yearly/custom counterpart — same attribution pipeline (via a shared internal `attributeMoves`), sourced from `ticker_price_history` (see Data Model) instead of `previous_close`, with move-size bars scaled per timeframe (`MAJOR_MOVE_STOCK_PCT_BY_TIMEFRAME`/`MAJOR_MOVE_PORTFOLIO_PCT_BY_TIMEFRAME` — a 5% daily stock move is notable, a 5% yearly move isn't). A stock with no `ticker_price_history` row old enough for the requested window is simply excluded from it, which is what makes longer-timeframe cards phase in gradually as history accumulates.
- **`generatePortfolioExplanation(slot, options)`** takes a `PortfolioInsightSlot` (no more zero-arg call) — same orchestration shape as before (load assets → compute the slot's own move → check the stored row for that exact slot → cache hit or fetch headlines + call the LLM → upsert), narrowed to just that slot's own story via an internal `scopeMoversResult` (a 'stock' slot's "MoversResult" is just that one mover; a 'sector' slot's is just that one theme + its member movers). `shouldRegenerate(current, last, resetsDaily = true)` gained the `resetsDaily` param — only `timeframe === 'daily'` resets on a new market day regardless of swing size; the rolling weekly/monthly/yearly/custom windows have no such discrete boundary and rely purely on the hysteresis/membership checks.
...
- **The Home page teaser costs no LLM call** — `buildTeaser()` is pure JS over `computeMovers()`'s output, plus one cheap read of the stored `portfolio_explanations` row (just to tell "you already saw this" from "something new happened," see below). Home already has `assets`/net worth loaded, so the card recomputes its one line on every render. Returns `null` (card renders nothing) when there's no major move.
- **`shouldRegenerate()` checks three independent levels — portfolio, sector, and individual stock — any one is enough on its own to trigger a fresh explanation**, not just the aggregate swing: (a) today's aggregate swing has moved meaningfully past what's stored, or flipped sign (`REGENERATION_HYSTERESIS_PCT`); (b) the *set* of individually-significant movers (`MAJOR_MOVE_STOCK_PCT`) differs from what's stored — a new holding joining the story, even with a similar-looking aggregate swing; (c) the *set* of flagged theme/sector moves (`isBroadMarketMove`'s sibling, theme-level) differs from what's stored — same idea, sector-scoped; (d) the stored explanation is from an earlier market day — `portfolio_explanations.market_date` (compared against `todayMarketDate()`, a plain UTC calendar-day key, same granularity as `net_worth_snapshots.date`/`tickers.last_updated` elsewhere in this app) resets it at the next market open regardless of how small the swing looks so far.
- **The teaser calls out specifically what's new, not a repeat of what you already read.** When the stored explanation is from today but `shouldRegenerate()` says it's now stale, `buildTeaser()` names the newly-flagged sector ("New activity in Semiconductors since your last check...") or newly-significant mover ("NVDA just moved...") rather than the generic first-time framing — falling back to a plain "changed since your last check" line only when it's a portfolio-level-only change with no new sector/stock to name. **Sector is a first-class framing on the first check of the day too, not just in the "what's new" path above** — `buildTeaser()`'s generic branch picks, in order: one dominant holding ("CRM moved +4.10% today..."), otherwise a flagged sector/theme move ("Your Semiconductors holdings moved up 4.33% today...", picking the theme with the largest average move when more than one is flagged), otherwise several individually-significant movers ("5 items in your portfolio moved substantially..."), otherwise the aggregate swing alone ("Your portfolio went up 1.20% today..."). Both branches now agree that a named sector outranks a plain mover count.
- **A regenerated explanation carries the still-relevant parts of the earlier one forward, rather than replacing it outright** — e.g. a noon explanation about one stock, then a semiconductor-sector move at 3pm, produces an explanation covering both, not just the sector move alone. `generatePortfolioExplanation()` passes the prior same-market-day summary (via `stripSources()` — its own "Sources" list is irrelevant/rebuilt fresh) to the LLM as context, and `EXPLANATION_SYSTEM_PROMPT` instructs it to fold still-current content in rather than dropping it or just concatenating the two. Only within the same market day — a prior day's story is dropped entirely, not folded into today's.
