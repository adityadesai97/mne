// Ported from fluid-functionalism (registry/default/lib/font-weight.ts) —
// verbatim. Requires a variable font (Inter) supporting `wght`/`opsz` axes to
// animate smoothly; mne doesn't load Inter Variable, so this degrades
// gracefully to whatever the active font's nearest static weight is —
// harmless, just not the buttery cross-fade the source site gets.
//
